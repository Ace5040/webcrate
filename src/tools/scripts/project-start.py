...
WEBCRATE_PROJECTS_FOLDERS = os.environ.get('WEBCRATE_PROJECTS_FOLDERS', '1000')
...
for projectname,project in projects.items():
  if PROJECT_NAME == projectname:
    ...
    if os.popen(f'docker container inspect {container_name} >/dev/null 2> /dev/null').read().strip():
      ...
      os.system(f'docker run -d --env-file=/webcrate-readonly/.env --hostname {container_name} --name {container_name} '
